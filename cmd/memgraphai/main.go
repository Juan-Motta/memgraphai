...
func main() {
	...
	if len(os.Args) < 2 || (os.Args[1] != "project" && os.Args[1] != "workstream" && os.Args[1] != "session" && os.Args[1] != "document" && os.Args[1] != "checkpoint" && os.Args[1] != "--library" && os.Args[1] != "--json" && os.Args[1] != "--operation-id") {
...
func runMCP(explicit string) {
	...
	defer store.Close()
	files := revisionfs.New(root, nil)
	if err := mcpadapter.RunStdio(context.Background(), app.ProjectService{Store: store}, app.ContinuityService{Store: store}, app.DocumentService{Store: store, Files: files}, app.Service{Recorder: store}, app.CheckpointService{Store: store, Files: files}); err != nil {
